...
mongoose.connect(process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/apex', { useNewUrlParser: true, useUnifiedTopology: true })
...
app.post('/api/upload-resume', upload.single('resume'), async (req, res) => {
    try {
        ...
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent?key=undefined`, {
            method: 'POST',
    ...
    (async () => {
        try {
            ...
            for (const lead of leads) {
                ...
                await runApplicationAgent(lead, profile, io);
            }
...
httpServer.listen(PORT, () => console.log(`✅ APEX Server running on port ${PORT}`));